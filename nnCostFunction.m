...
for i=1:m
  y_recoded=zeros(num_labels,1); %Recode the labels as vectors containing only values 0 or 1.
  y_recoded(y(i))=1;
  z2=Theta1*X(:,i);
  a2=sigmoid(z2);
  ...
  delta2=delta2(2:end);
  Theta2_grad+=(1/m)*delta3*[1,a2'];
  Theta1_grad+=(1/m)*delta2*X(:,i)';
  J+=(-1/m)*sum(y_recoded.*log(h)+(1-y_recoded).*log(1-h));
endfor
